...
int thirdSensor = 0;
int fourSensor = 0;// digital sensor
int fiveSensor = 0;
int sixSensor = 0;
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    secondSensor = analogRead(1)*2;
    // read  switch, map it to 0 or 255L
    thirdSensor = analogRead(2)*4;
    fourSensor = analogRead(3)*6;
    fiveSensor = analogRead(4)*3;
     sixSensor = analogRead(5)*3;
    // send sensor values:
    Serial.write(firstSensor);
    Serial.write(secondSensor);
    Serial.write(thirdSensor);
    Serial.write(fourSensor);
    Serial.write(fiveSensor);
